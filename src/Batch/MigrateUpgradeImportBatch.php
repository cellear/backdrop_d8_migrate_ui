...
namespace Drupal\migrate_backdrop_ui\Batch;
...
class MigrateUpgradeImportBatch {
   ...
   *
   * @var \Drupal\migrate_backdrop_ui\Batch\MigrateMessageCapture
   */
  ...
  public static function run($initial_ids, $config, &$context) {
    ...
    if ($migration) {
      ...
      try {
        ...
        \Drupal::logger('migrate_backdrop_ui')->error($e->getMessage());
      ...
      switch ($migration_status) {
        case MigrationInterface::RESULT_COMPLETED:
          ...
          \Drupal::logger('migrate_backdrop_ui')->notice($message);
          $context['sandbox']['num_processed'] = 0;
        ...
        case MigrationInterface::RESULT_FAILED:
          ...
          \Drupal::logger('migrate_backdrop_ui')->error('Operation on @migration failed', ['@migration' => $migration_name]);
          break;
        ...
        case MigrationInterface::RESULT_SKIPPED:
          ...
          \Drupal::logger('migrate_backdrop_ui')->error('Operation on @migration skipped due to unfulfilled dependencies', ['@migration' => $migration_name]);
          break;
      ...
      foreach (static::$messages->getMessages() as $message) {
        $context['sandbox']['messages'][] = (string) $message;
        \Drupal::logger('migrate_backdrop_ui')->error($message);
      }
  ...
  public static function finished($success, $results, $operations, $elapsed) {
    ...
    if (\Drupal::moduleHandler()->moduleExists('dblog')) {
      $url = Url::fromRoute('migrate_backdrop_ui.log');
